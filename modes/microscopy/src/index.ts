...
import segmentationButtons from './seg/segmentationButtons';
...
function modeFactory({ modeConfiguration }) {
  ...
  return {
    ...
    onModeEnter: function ({ servicesManager, extensionManager, commandsManager }: withAppTypes) {
      ...
      toolbarService.addButtons([...toolbarButtons, ...moreTools, ...segmentationButtons]);
      ...
      toolbarService.createButtonSection('segmentationToolbox', ['BrushTools', 'Reset', 'Save']);
    ...
    isValidMode: function ({ modalities }) {
      ...
      return {
        valid: !!modalities_list.filter(modality => NON_IMAGE_MODALITIES.indexOf(modality) === -1)
          .length,
        description:
          'The mode does not support studies that ONLY include the following modalities: SM, ECG, RTSTRUCT',
      };
    },
    routes: [
      {
        ...
        layoutTemplate: ({ location, servicesManager }) => {
          return {
            ...
            props: {
              ...
              rightPanels: [cornerstone.panelTool, tracked.measurements],
